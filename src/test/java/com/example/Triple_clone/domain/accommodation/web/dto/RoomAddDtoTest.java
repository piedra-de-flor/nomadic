...
class RoomAddDtoTest {

    private final Validator validator = Validation.buildDefaultValidatorFactory().getValidator();

    @Test
    @DisplayName("@NotBlank/@Min/@Size/@AssertTrue 검증")
    void 필드_검증() {
        RoomAddDto dto = RoomAddDto.builder()
                .name("")
                .dayusePrice(-1)
                .dayuseSalePrice(-1)
        ...
        Set<ConstraintViolation<RoomAddDto>> violations = validator.validate(dto);
    ...
    void 정상값_검증() {
        RoomAddDto ok = RoomAddDto.builder()
                .name("디럭스")
                .capacity(2)
                .maxCapacity(3)
                .build();
