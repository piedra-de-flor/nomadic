...
def run_selenium(location: str, db: Session):
    """ 야놀자 크롤링 & MySQL 저장 """
    try:
        ...
        for element in elements:
            name = element.select_one("div.PlaceListTitle_container__qe7XH").text.strip() if element.select_one("div.PlaceListTitle_container__qe7XH") else None
            score = float(element.select_one("span.PlaceListScore_rating__3Glxf").text.strip()) if element.select_one("span.PlaceListScore_rating__3Glxf") else None
            category = element.select_one("div.PlaceListGrade_container__1oIhJ").text.strip() if element.select_one("div.PlaceListGrade_container__1oIhJ") else None
            ...
            price_info_texts = [BeautifulSoup(str(e), "html.parser").text.strip() for e in price_info_elements if BeautifulSoup(str(e), "html.parser").text.strip()]
            ...
            accommodation = {
                ...
                if price_texts[0] and lent_parsed_info.get("status", True) and price_texts[0] != "예약마감" else None,
                "lent_status": lent_parsed_info.get("status", True),
                "enter_time": datetime.strptime(lodgment_parsed_info["time"], "%H:%M").time() if lodgment_parsed_info["time"] else datetime.strptime("00:00", "%H:%M").time(),
                "lodgment_discount_rate": lodgment_parsed_info.get("discount_rate") if lent_parsed_info["discount_rate"] else 0,
                "lodgment_origin_price": lodgment_parsed_info.get("origin_price") if lent_parsed_info["origin_price"] else 0,
                "lodgment_price": int(price_texts[1].replace(',', '').replace('원~', '').replace('최대할인가', ''))
                if price_texts[1] and lodgment_parsed_info.get("status", True) and price_texts[1] != "예약마감" else None,
...
def scrape_endpoint(location: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """ 비동기 크롤링 실행 """
