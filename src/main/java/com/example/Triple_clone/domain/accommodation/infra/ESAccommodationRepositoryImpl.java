...
public class ESAccommodationRepositoryImpl {
    ...
    public Page<AccommodationDocument> advancedNaturalLanguageSearch(
            SearchParams params,
            Pageable pageable
    ) {
        try {
            ...
            if (params.getSearchKeyword() != null && !params.getSearchKeyword().isEmpty()) {
                shouldQueries.add(QueryBuilders.multiMatch(m -> m
                        .fields("name^10", "address^3", "region^5", "category^2",
                                "intro^4", "info^3", "amenities^6", "landmarkDistance^2")
                        .query(params.getSearchKeyword())
                        .fuzziness("AUTO")
                        .type(TextQueryType.BestFields)
                        .boost(3.0f)
                ));

                shouldQueries.add(QueryBuilders.nested(n -> n
                        .path("rooms")
                ...
                shouldQueries.add(QueryBuilders.matchPhrase(m -> m
                        .field("name")
                        .query(params.getSearchKeyword())
                        .boost(5.0f)
                ));
                ...
                shouldQueries.add(QueryBuilders.multiMatch(m -> m
                        .fields("name.edge^3", "name.ngram^2")
                        .query(params.getSearchKeyword())
                        .boost(1.5f)
                ));
            }
            ...
            if (params.getCategory() != null) {
                mustQueries.add(QueryBuilders.term(t -> t.field("category").value(params.getCategory())));
            }
            if (params.getRatingMin() != null) {
                mustQueries.add(QueryBuilders.range(r -> r.field("rating").gte(JsonData.of(params.getRatingMin()))));
            }
            if (params.getRatingMax() != null) {
                mustQueries.add(QueryBuilders.range(r -> r.field("rating").lte(JsonData.of(params.getRatingMax()))));
            }
            if (params.getRegion() != null) {
                mustQueries.add(QueryBuilders.term(t -> t.field("region").value(params.getRegion())));
            }

            if (params.getRoomPriceMin() != null || params.getRoomPriceMax() != null) {
                List<Query> priceQueries = new ArrayList<>();

                if (params.getRoomPriceMin() != null) {
                    priceQueries.add(QueryBuilders.bool(b -> b
                            .should(QueryBuilders.range(r -> r.field("rooms.dayuse_price").gte(JsonData.of(params.getRoomPriceMin()))))
                            .should(QueryBuilders.range(r -> r.field("rooms.stay_price").gte(JsonData.of(params.getRoomPriceMin()))))
                            .minimumShouldMatch("1")
                    ));
                }

                if (params.getRoomPriceMax() != null) {
                    priceQueries.add(QueryBuilders.bool(b -> b
                            .should(QueryBuilders.range(r -> r.field("rooms.dayuse_price").lte(JsonData.of(params.getRoomPriceMax()))))
                            .should(QueryBuilders.range(r -> r.field("rooms.stay_price").lte(JsonData.of(params.getRoomPriceMax()))))
                            .minimumShouldMatch("1")
                    ));
                }
                ...
                mustQueries.add(QueryBuilders.nested(n -> n
                        .path("rooms")
                        .query(q -> q.bool(b -> b.must(priceQueries)))
                ));
            }

            if (params.getRoomCapacityMin() != null) {
                mustQueries.add(QueryBuilders.nested(n -> n
                        .path("rooms")
                        .query(q -> q.range(r -> r.field("rooms.capacity").gte(JsonData.of(params.getRoomCapacityMin()))))
                ));
            }

            if (params.getHasDiscount() != null && params.getHasDiscount()) {
                shouldQueries.add(QueryBuilders.nested(n -> n
                        .path("rooms")
                        .query(q -> q.bool(b -> b
                                .should(QueryBuilders.term(t -> t.field("rooms.has_dayuse_discount").value(true)))
                                .should(QueryBuilders.term(t -> t.field("rooms.has_stay_discount").value(true)))
                                .minimumShouldMatch("1")
                        ))
                        .boost(1.2f)
                ));
            ...
            if (params.getLateCheckout() != null && params.getLateCheckout()) {
                shouldQueries.add(QueryBuilders.nested(n -> n
                        .path("rooms")
                        .query(q -> q.bool(b -> b
                                .should(QueryBuilders.terms(t -> t
                                        .field("rooms.stay_checkout_time")
                                        .terms((TermsQueryField) Arrays.asList(
                                                FieldValue.of("13:00"),
                                                FieldValue.of("14:00"),
                                                FieldValue.of("15:00"),
                                                FieldValue.of("16:00"),
            ...
            if (params.getFacilities() != null && !params.getFacilities().isEmpty()) {
                for (String facility : params.getFacilities()) {
                    shouldQueries.add(QueryBuilders.match(m -> m
                            .field("amenities")
                            .query(facility)
                            .boost(1.5f)
                    ));
            ...
            if (params.getLocationContext() != null && !params.getLocationContext().isEmpty()) {
                for (String location : params.getLocationContext()) {
                    shouldQueries.add(QueryBuilders.multiMatch(m -> m
                            .fields("name^3", "address^2", "landmarkDistance^4", "intro^2")
                            .query(location)
                            .boost(2.0f)
                    ));
                }
            ...
            if (params.getRoomTypes() != null && !params.getRoomTypes().isEmpty()) {
                for (String roomType : params.getRoomTypes()) {
                    shouldQueries.add(QueryBuilders.nested(n -> n
                            .path("rooms")
                            .query(q -> q.match(m -> m
                                    .field("rooms.name")
            ...
            SearchRequest searchRequest = SearchRequest.of(s -> s
                    ...
                    .highlight(h -> h
                            .fields("name", hf -> hf.preTags("<mark>").postTags("</mark>"))
                            .fields("address", hf -> hf.preTags("<mark>").postTags("</mark>"))
                            .fields("intro", hf -> hf.preTags("<mark>").postTags("</mark>"))
                    )
            );
            ...
            List<AccommodationDocument> results = response.hits().hits().stream()
                    .map(hit -> {
                        AccommodationDocument doc = hit.source();
                        if (hit.highlight() != null) {
                            log.debug("검색어 하이라이팅: {}", hit.highlight());
                        }
                        return doc;
                    })
            ...
            log.info("자연어 검색 완료: 쿼리='{}', 결과={}건", params.getSearchKeyword(), total);
            return new PageImpl<>(results, pageable, total);

        } catch (IOException e) {
            log.error("고급 검색 오류: {}", e.getMessage(), e);
            throw new RuntimeException("고급 검색 오류: " + e.getMessage(), e);
        }
    }
